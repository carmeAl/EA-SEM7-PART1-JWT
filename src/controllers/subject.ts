...
import { insertSubject,getSubject,getSubjects,updateSubject,deleteSubject, matriculateSubject,usersInSubject} from "../services/subject";
...
const usersIn_Subject = async({params}:Request,res:Response)=>{
    try{
        const {idSubject} = params;
        const response = await usersInSubject(idSubject);
        const data = response ? response:"NOT_FOUND";
        res.send(data);
    } catch(e){
        handleHttp(res,"ERROR_GET_SUBJECT");
...
export{get_Subject,get_Subjects,post_Subject,update_Subject,delete_Subject,matriculate_Subject,usersIn_Subject};